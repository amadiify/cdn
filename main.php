...
{
	/**
		Changes made in line : 424. I altered the default method for generating activation code.
	...
	**/
	...
	public function postUser($action)
	{
		if (!is_null($action))
		{
						...
						if (is_object($http->responseJson))
						{
							...
							if ($status)
							{
								...
								$this->status('success')
									...
									->email($user->Email)
									->token($token)
									->profileimage($table->profile)
									->ok;
							}
							else
							{
								$msg = isset($http->responseJson->msg) ? $http->responseJson->msg : "Something went wrong";

								$this->status('Error')->message($msg)->ok;
							}
						}
						else
						{
							$this->status('Error')->message('Authentication Failed. Please try again.')->ok;
						}
					}
					else
					{
						$this->status('Error')->message('Invalid POST Fields')->ok;
					}
				});
			}
			elseif ($action == 'register')
			{
				...
				if (!is_set($query))
				{
					$this->status('Error')->message('Invalid Registration POST Fields')->ok;
				}
				else
				{
					is_set($query, function($data) use ($http){
						...
						$phone = DB::Customers(get()->where('PhoneNumber = ?')->bind($data->PhoneNumber));

						if ($phone->rows == 0)
						{
							// check for email address
							$email = DB::Customers(get()->where('PhoneNumber = ? and EmailAddress = ?')->bind($data->PhoneNumber, $data->EmailAddress));
							...
							if ($email->rows == 0)
							{
								...
								else
								{
									...
									{
										$this->status('Success')->message('Registration successful. You can login to your ewallet with your Email/Phonenumber and password.')->ok;
									}
									else
									{
										$this->status('Pending')->message('Request waiting, You may have to resubmit later.')->ok;
									}
								}
							}
							else
							{
								$this->status('Error')->message('Email address already in use.')->ok;
							}
						}
						else
						{
							$this->status('Error')->message('Phone number already in use')->ok;
						}
			...
			elseif ($action == 'reset')
			{
				is_set('username:post|EmailAddress:post', function($post) use ($action){
					...
					if ($exists->rows > 0 )
					{
						...
						{
							$this->status('Error')->message("Operation failed. Please try again")->ok;
						}
					}
					else
					{
						$this->status('Failed')->message("Invalid User Account.")->ok;
					}
				});
	...
	public function getCheckUser()
	{
		is_set('username:get', function($e){
			$user = DB::AspNetUsers(get()->where('UserName = ? or PhoneNumber = ?')->bind(strip_tags($e->username), strip_tags($e->username)));
			...
			if ($user->rows > 0)
			{
				...
				$exists = $this->mysql->aspnetusers(get('UserName = ? or UserName = ?')->bind($e->username, $user->PhoneNumber));
				...
				if ($exists->rows > 0)
				{
					...
					$this->status('Success')->message($message)->header('Welcome Back '.$user->Lastname)->ok;
				}
				else
				{
					$this->status('Success')->message("Howdy! How are you today? We hope you are doing great! Your friends from esusuonline.")->header('Welcome '.$user->Lastname)->ok;	
				}
			}
			else
			{
				$this->status('Error')->message('Invalid User. Please check your login and try again')->ok;
			}
		});

		if (!is_set('username:get'))
		{
			$this->status('Error')->message('Request parameter not set.')->ok;
		}
	}
	...
	public function postDevice($meth)
	{
		if ($meth == 'check')
		{
			is_set('username:post', function($e){
				...
				if ($user->rows > 0)
				{
					...
					if ($uid->rowCount() > 0)
					{
						...
						if ($device->rowCount() > 0)
						{
							...
							{
								$this->status('Success')->message('Activated')->ok;
							}
							else
							{
								$this->status('Error')->message('Not activated')->ok;
							}
						}
						else
						{
				...
				{
					$this->status('Error')->message('Invalid User Account. Please check and try again.')->ok;
				}
		...
		elseif ($meth == 'activate')
		{
				...
				if ($user->rows > 0)
				{
					...
					if ($uid->rowCount() > 0)
					{
						...
						if ($user->rows > 0)
						{
							...
							Plugins::sms()->send('EsusuOnline', 'Enter '.$code.' to activate your device.', $phone);
							...
							$this->status('Success')->build($build)->ok;
						}
						else
						{
							$this->status('Error')->message('Invalid User Account. Please check and try again.')->ok;
			...
			is_set('userid:post', function($e){
				...
				if ($diff->i < 15)
				{
					...
					$this->status('Success')->message('Device Registered.')->ok;
				}
				else
				{
					$this->status('Error')->message('Code expired. Sorry, but you have to make a new request.')->ok;
				}
			});
		}
		elseif ($meth == 'login')
		{
			is_set('username:post', function($e){
				...
				$this->mysql->aspnetusers(update($time)->where('UserName = :uname or PhoneNumber = :phone')->bind($e->username, $e->username));

				$this->status('Success')->message('logged in')->ok;
			});
		}
	}
	...
	public function getUser($id)
	{
		...
		{
			$userid = $_GET['userid'];
			$customer = DB::Customers(get()->where('UserProfileID = ?')->bind($userid));
			if ($customer->rows > 0)
			{
		...
		if (is_null($id))
		{
			$user = DB::AspNetUsers(get()->where('UserName = ?')->bind(session('api.request.user')));

			$id = $user->Id;

			// get customer info
			$customer = DB::Customers(get()->where('UserProfileID = ?')->bind($id));
			...
			else
			{
				...
				$cus = [
						...
						'PhoneNumberConfirmed' => $customer->IsPhoneNumberConfirmed,
						'KYCComplete' => $customer->IsKYCComplete,
						...
						'NextOfKinPhoneNumber' => $customer->NextOfKinPhoneNumber,
						'NextOfKinOccupation' => $customer->NextOfKinOccupation,
			...
			if (count($data) > 0)
			{
				$this->status('Success')->data($data)->ok;
			}
			else
			{
				$this->status('Error')->message('Request Failed. Please try again.')->ok;
			}
		}
		else
		{
			// get customer info
			$customer = DB::Customers(get()->where('ID = ?')->bind($id));

			if ($customer->rows > 0)
			{
				$user = DB::AspNetUsers(get()->where('UserProfileID = ?')->bind($customer->UserProfileID));
			...
			{
				$this->status('Error')->message('Invalid CustomerID')->ok;
			}
		}
	}
	...
	public function putAgent(string $action, $cid = null)
	{
		...
		if ($action == 'request')
		{
			if (isset($info->data->ID))
			{
				...
				$isRequested = DB::AgentRequests(get()->where('CustomerID = ?')->bind($cid));

				if ($isRequested->rows == 0)
				{
					// check Agent lists
					$Customer = DB::Customers(get()->where('ID = ?')->bind($cid));

					$info = mw::customer()->info($cid);

					if ($info->data->IsKYCComplete == 1)
					{

						if (!is_null($Customer) && $Customer->rows > 0)
						{
							$profileID = $Customer->UserProfileID;
							$isAgent = DB::Agents(get()->where('UserProfileID = ?')->bind($profileID));
							...
							if (!is_null($isAgent) && $isAgent->rows == 0)
							{
								...
								if ($insert !== false)
								{
									if ($insert->ok)
									{
										$this->status('Success')->message('Your request to be an agent has been submitted.')->ok;
									}
								}
								else
								{
									$this->status('Error')->message('Unable to make request at this time, please try again later.')->ok;
								}
							}
							else
							{
								// already an agent
								$this->status('Error')->message('Already an Esusu Agent. Cannot make a request again.')->ok;
							}
						}
						else
						{
							// not a valid user
							$this->status('Error')->message('Account Verification failed.')->ok;
						}
					}
					else
					{
						$this->status('Error')->message('KYC not Complete. Cannot make request.')->ok;
					}
				}
				else
				{
					// made a request previously
					$this->status('Error')->message('Already made a request to be an agent.')->ok;
				}
			}
			else
			{
				$this->status('Error')->message("Invalid Esusu Customer Account!")->ok;
			}
		}
		elseif ($action == 'liquidate')
		{
			...
			if (isset($info->data->ID))
			{
				is_set('SerialNumber:post&VoucherCode:post&CustomersPhoneNo:post&CustomersPIN:post', function($data) use ($info){
					...
					if (is_object($http->responseJson))
					{
						if (isset($http->responseJson->status))
						{
							$status = $http->responseJson->status == true ? 'Success' : 'Error';
						}
						else
						{
							$status = 'Error';
						...
						$this->status($status)->message($http->responseJson->msg)->ok;
					}
					else
					{
						$this->status('Error')->message('Voucher Liquidation failed!')->ok;
					}
				});
		...
		{
			$this->status('Error')->message('Invalid Api request '. $action)->ok;
		}
	}
	...
	public function getAgent($action, $cid = null)
	{
		if ($action == 'status')
		{
			if (is_null($cid))
			{
				if(session('api.userid'))
				{
					...
					// revalidate user
					$check = DB::AspNetUsers(get()->where("ID = ?")->bind($id));
					...
					if ($check->rows > 0)
					{
						...
						{
							$this->status('Error')->message('Customer account not found. Cannot make request')->ok;
						}
					}
					else
					{
						$this->status('Error')->message('Invalid Esusu User account.')->ok;
					}
				}
				else
				{
					$this->status('Error')->message('User not authenticated.')->ok;
				}
			}
			else
			{
				// get agent information
				$agent = DB::AgentRequests(get()->where('CustomerID = ?')->bind((int) $cid));

				if ($agent->rows > 0)
				{
					$this->status($agent->Status)->dateRequested($agent->DateRequested)->dateUpdated($agent->DateUpdated)->ok;
				}
				else
				{
					$this->status('Error')->message('Not an Esusu Agent, please make a request.')->ok;
				}
			}
		}
		...
		elseif (is_numeric($action))
		{
			...
			$cus = DB::Customers(get()->where('ID = ?')->bind((int) $action));
			...
			if ($cus->rows > 0)
			{
				...
				$agent = DB::Agents(get()->where('UserProfileID = ?')->bind($UserProfileID));

				if ($agent->rows > 0)
				{
					$this->status('Success')->data($agent->object)->ok;
				}
				else
				{
					// not an agent
					$this->status('Error')->message('Customer not an Esusu Agent.')->ok;
				}
			}
			else
			{
				$this->status('Error')->message('Invalid Customer ID.')->ok;
			}
		}	
		else
		{
	...
	public function getAccountInfo($cid)
	{
		...
		$plan = isset($savings->data->plan) ? $savings->data->plan : null;


		echo json_encode(['plans' => $plan,
			'allreport' => $reports->report ?? null,
			'bank' => $bank,
			'profile' => $customer,
			'balance' => $balance->balance,
			'accountNumber' => $balance->accountNumber,
			'allplans' => $allplans,
			'allplansreport' => $allplansreport,
			'interest' => $interest]);
	}
	...
	public function getCustomer($action = 'view', $cid = null)
	{
		if ($action == 'view')
		{
			if (!is_null($cid))
			{
				...
				$info->data->DateCreated = date('Y-m-d g:i a', strtotime($info->data->DateCreated->date));

				if ($info->ok)
				{
					$this->status('Success')->data($info->data)->ok;
				}
				else
				{
					$this->status('Error')->message($info->message)->ok;
				}
			}
			else
			{
		...
		elseif ($action == 'balance')
		{
			...
			$Account = DB::CustomerMobileAccounts(get()->where('CustomerID = ?')->bind($ID));

			if ($Account->row > 0)
			{
				$this->status('Success')->balance($this->convertToNaria($Account->Balance))->bvn($info->data->BVN)->accountNumber($Account->AccountNumber)->ok;
			}
			else
			{
				$this->status('Error')->message('Invalid Customer Account.')->ok;
			}
		...
		elseif ($action == 'plan')
		{
			...
			if ($check->row > 0)
			{
				...
				while($c = $check->object())
				{
					...
					$product = DB::Products(get()->where('ID = ?')->bind($ProductID));

					if ($product->row > 0)
					{
						$created = new DateTime($c->DateCreated->date);
						$due = new DateTime($c->DueDate->date);

						$dueTimeStamp = $due->getTimeStamp();
						$current = new DateTime();
						$currentTimestamp = $current->getTimeStamp();

						$interest = DB::Interests(get()->where('ID = ?')->bind($product->InterestID));
				...
				$this->status('Success')->data(['plan' => array_merge($activePlans, $allPlans)])->ok;

			}
			else
			{
				$this->status('Error')->message('Customer do not have an active plan.')->ok;
			}
		}
		elseif ($action == 'interest')
		{
			$mobileAccount = DB::CustomerMobileAccounts(get()->where('CustomerID = ?')->bind($cid));

			if ($mobileAccount->row > 0)
			{
				$MID = $mobileAccount->ID;	

				$intrest = DB::CustomerProductInterests(get()->where('CustomerMobileAccountID = ?')->bind($MID));
				...
				if ($intrest->rows > 0)
				{
					...
					while($in = $intrest->object())
					{
						...
						{
							$product = DB::Products(get()->where('ID = ?')->bind($in->CustomerProductID));

							$data[$product->Name] = [];
							$data[$product->Name]['duration'] = $product->Duration;
							$data[$product->Name]['amount'] = $this->convertToNaria($in->Amount);

							$intr = DB::Interests(get()->where('ID = ?')->bind($product->InterestID));
					...
					{
						$this->status('Error')->message('No Active Product Interest')->ok;
					}
				}
				else
				{
					$this->status('Error')->message('No Product Interest.')->ok;
				}
			}
			else
			{
				$this->status('Error')->message('Customer doesn\'t have a MobileAccount.')->ok;
			}
		...
		elseif ($action == 'bank')
		{
			...
			$account = DB::CustomerBankAccounts(get()->where('CustomerID = ?')->bind($ID));
			...
			if ($account->rows > 0)
			{
				...
				// get bank
				$bank = DB::Banks(get()->where('ID = ?')->bind($bankid));

				$this->status('success')->bank($bank->BankName)->accountName($account->AccountHolderName)->accountNumber($account->AccountNumber)->ok;
			}
			else
			{
				$this->status('error')->message('Invalid Customer account.')->ok;
		...
		elseif ($action == 'withdrawal')
		{
			...
			$account = DB::CustomerBankAccounts(get()->where('CustomerID = ?')->bind($info->data->ID));
			$mobileAccount = DB::CustomerMobileAccounts(get()->where('CustomerID = ?')->bind($info->data->ID));	
			...
			if ($account->rows > 0)
			{
				...
				{
					$wallet = DB::sql("SELECT * FROM RealTimePostings WHERE AccountNumber = ? OR AccountNumber = ? ORDER BY ID DESC OFFSET 0 ROWS FETCH NEXT $limit ROWS ONLY")->bind($AccountNumber, $mobileAccount->AccountNumber)->run();	
				}
				else
				{
			...
			$this->status('Success')->report($record)->ok;
		}
		else
		{
			$this->status('Error')->message('Invalid Api request '. $action)->ok;
	...
	public function postTransfer($cid=null)
	{
		...
		if ($info->ok)
		{
			...
			is_set('RecepientAccountNumber:post&DepositAmount:post&BankName:post&PIN:post', function($post) use (&$http, &$info){
				...
				$post->CustomerID = $info->data->ID;
				// send 
				$http->post($post)->send();

				if (is_object($http->responseJson))
				{
					$msg = $http->responseJson->msg;
					$status = $http->responseJson->status;
		...
		{
			$this->status('Error')->message('Authentication required!')->ok;
		}
	}
	...
	{	
		...
		if (count($args) > 0)
		{
			...
			foreach($args as $i => $x)
			{
				...
				{
					$type = is_bool($x) ? PDO::PARAM_BOOL : 
							is_string($x) ? PDO::PARAM_STR : 
							is_int($x) ? PDO::PARAM_INT : '';
		...
		if (preg_match('/(select)/i', $sql))
		{
			...
			{
				while($r = $objref->fetchAll(PDO::FETCH_ASSOC)){ $object[] = (object) $r;}
	...
	public function postCustomer($action = 'edit', $cid = null)
	{
			...
			if (count($data) > 0)
			{
				...
				if ($info->ok)
				{
					...
					if ($fields->ok)
					{
						...
						{
							// allow all
							$update = DB::Customers(update($data)->where('ID = ?')->bind($cid));

							if ($update->ok)
							{
								$this->status('Success')->message('Edit successfull.')->ok;
							}
							else
							{
								$this->status('Error')->message('Edit failed. Please try again later, server may be busy')->ok;	
							}
						}
						else
						{
							...
							{
								// all failed.
								$this->status('Error')->message('Failed updating column[s] {'.implode('|', $failed).'}')->reason('Not Allowed')->ok;
							}
							elseif (count($failed) > 0 && count($cols) > 0)
							{
								// update method here..
								$update = DB::Customers(update($cols)->where('ID = ?')->bind($cid));

								if ($update->ok)
								{
									$this->status('Success')->message('Edit successfull.')->message2('Failed updating column[s] {'.implode('|', $failed).'}')->reason('Not Allowed')->ok;	
								}
								else
								{
									$this->status('Error')->message('Edit failed. Please try again later, server may be busy')->ok;
					...
					{
						$this->status('Error')->message($fields->message)->ok;
					}
				}
				else
				{
					$this->status('Error')->message("Customer Account not found.")->ok;
				}
			}
			else
			{
				$this->status('Error')->message('No POST data sent')->ok;
			}
		}
		elseif ($action == 'changepin')
		{
			is_set('CurrentPin:post', function($post) use ($cid){
				...
				{
					$this->status('Error')->message('Invalid Post fields. Please see documentation.')->ok;
				}
			});
		...
		elseif ($action == 'changepassword')
		{
			is_set('CurrentPassword:post&NewPassword:post&ConfirmPassword:post', function($post)
			{
				...
				if (is_object($http->responseJson))
				{
					...
					if ($status)
					{
						$this->status('Success')->message($msg)->ok;
					}
					else
					{
						$this->status('Error')->message($msg)->ok;
			...
			{
				$this->status('Error')->message('An error occured. POST Data sent, please try again')->ok;
			}
		}
		elseif ($action == 'profile')
		{
			if (isset($_FILES['profile']))
			{
				...
				if (move_uploaded_file($tmp, $dest))
				{
					...
					$user = $table->update(['profile' => $name], 'UserName = :un or UserName = :un2', $info->data->EmailAddress, $info->data->PhoneNumber);

					$this->status('Success')->message('Upload successfull')->ok;
				}
				else
				{
					$this->status('Error')->message('Upload failed. Please try again later')->ok;
				}
			}
			else
			{
		...
		if ($action == 'add')
		{
			...
			if ($role->Name != 'Customer')
			{
				if (count($data) > 0)
				{
					...
					if ($insert->ok)
					{
						$this->status('Success')->message('Customer information added successfully')->ok;
					}
					else
					{
						$this->status('Error')->message('Failed! Could\'t submit POST Data.')->error(DB::$handler->error())->ok;
					}
				}
				else
				{
					$this->status('Error')->message('No POST Data sent. Please try again')->ok;
				}
			}
			else
			{
				$this->status('Error')->message('You do not have such permission.')->ok;
			}
		}
		elseif($action == 'bank')
		{
			if (is_set('AccountHolderName:post'))
			{
				is_set('AccountHolderName', function($data) use($cid){
					...
					{
						// check bank
						$banks = DB::Banks(get()->where('ID = ?')->bind($data->BankID));
						...
						if ($banks->rows > 0)
						{
							...
							$check = DB::CustomerBankAccounts(get()->where('CustomerID = ?')->bind($info->data->ID));

							if ($check->rows > 0)
							{
								$this->status('error')->message('You have already added a bank account')->ok;
							}
							else
							{
								$data->CustomerID = $info->data->ID;
								$data->Status = 1;
								...
								if ($add !== false)
								{
									$this->status('success')->message('Bank Account was added successfully')->ok;
								}
								else
								{
									$this->status('error')->message('Failed. Please try again')->ok;
								}
							}
						}
						else
						{
							$this->status('error')->message('Invalid Bank ID')->ok;
						}
					}
					else
					{
						$this->status('error')->message('Invalid POST fields. See documentation')->ok;
					}
				});	
			}
			else
	...
	public function getBank($cid = null)
	{
		...
		{
			$cid = $info->data->ID;
			$bank = DB::CustomerBankAccounts(get()->where('CustomerID = ? and Status = ?')->bind($cid, 1));

			if ($bank->rows > 0)
			{
				$bank = $bank->object;
				// get bank
				$getBank = DB::Banks(get()->where('ID = ? and Status = ?')->bind($bank->BankID, 1));

				if ($getBank->rows > 0)
				{
					$bankName = $getBank->BankName;

					$data = ['AccountHolderName' => $bank->AccountHolderName,
				...
				{
					$this->status('Error')->message('Bank doesn\'t exist.')->ok;
				}
			}
			else
			{
				$this->status('Error')->message('No bank account added.')->ok;
			}
		}
	}
	...
	public function getBanks($bankid = null)
	{
		if (!is_null($bankid))
		{
			$banks = DB::Banks(get()->where('ID = ? and Status = ?')->bind($bankid, 1));
		}
		else
		{
			$banks = DB::Banks(get()->where('Status = ?')->bind(1));
		}
		...
		if ($banks->row > 0)
		{
			if ($banks->row == 1)
			{
				$this->status('success')->bankName($banks->BankName)->code($banks->Code)->ok;
			}
			else
			{
				$bnk = [];
				...
				$this->status('success')->data($bnk)->ok;
			}
		}
		else
		{
	...
	public function getVerify($type, $cid = null)
	{
		...
		if ($type == 'phone')
		{
			if ($info->data->IsPhoneNumberConfirmed == 0)
			{
				$post = [];
				$post['id'] = $info->data->ID;
				$post['phone'] = $info->data->PhoneNumber;
				$post['Message'] = '3';
				...
				if (is_object($http->responseJson))
				{
					$this->status('Success')->message($http->responseJson)->ok;
				}	
				else
				{
					$this->status('Error')->message("Operation failed. Please try again")->ok;
		...
		if ($type == 'kyc')
		{
			if ($info->data->IsKYCComplete == 0)
			{
				...
				if (isset($_FILES['Passport']) && isset($_FILES['UtilityBill']) && isset($_FILES['Identification']))
				{
					...
					// check KYCUploads
					$uploads = DB::KYCUploads(get()->where('CustomerID = ?')->bind($info->data->ID));
					...
					{
						$this->status('pending')->message('Your doucments are currently being reviewed.')->ok;
					}
				}
				else
				{
			...
			{
				$this->status('Error')->message('Your KYC has already been Verified')->ok;
			}
		}
		elseif ($type == 'bank')
		{
			if ($info->data->IsBankAccountConfirmed == 0)
			{
				is_set('BankID:post', function($post) use ($info, $cid){
					if (isset($post->AccountHolderName) && isset($post->AccountNumber) && isset($post->PIN))
					{
						...
						if (is_object($http->responseJson))
						{
							...
							if ($msg == 'true')
							{
								$this->status('Success')->message('Bank Verified successfully')->ok;
							}
							else
							{
								$this->status('Error')->message($msg)->ok;
			...
			{
				$this->status('Error')->message('Bank Account Verified')->ok;
			}
		}
		elseif ($type == 'phone')
		{
			...
			{
				$this->status('Error')->message('Your phone number has been verified.')->ok;
			}
		}
		else
		{
			$this->status('Error')->message('Invalid Api request '. $type)->ok;
		}
	}
	...
	public function getSaving($action, $cid = null, $method = null)
	{
		...
		if ($action == 'plans')
		{
			...
			{
				// Get plans
				$plans = DB::Products(get()->where('Status = ?')->bind(1));
				...
				if ($plans->rows > 0)
				{
					...
					{
						$interest = DB::Interests(get()->where('ID = ?')->bind($d->InterestID));
					...
					$this->status('Success')->plans($data)->ok;
				}
				else
				{
					$this->status('Error')->message('No Savings Plan Avaliable please try again later.')->ok;
				}
			}
		}
		elseif ($action == 'report')
			...
			{	
				...
				if ($check->row > 0)
				{
					while($c = $check->object())
					{
						...
						$product = DB::Products(get()->where('ID = ?')->bind($c->ProductID));
		...
		elseif ($action == 'interest')
		{
			...
			if ($check->row > 0)
			{
				...
				while($c = $check->object())
				{
					...
					// get interest
					$interest = DB::sql("SELECT RealTimePostings.AccountNumber, RealTimePostings.Amount, RealTimePostings.Narration FROM RealTimePostings where Narration LIKE '%Credit Interest%' AND AccountNumber = ?")->bind($c->Code)->run();
					...
					$data[$c->Code] += $this->getAccuredInterest($c->ID);
				...
				$this->status('Success')->interest($data)->ok;
			}
			else
			{
				$this->status('Error')->interest([])->ok;
			}
	...
	public function getAccuredInterest($ProductID, $hide = true)
	{
		...
		if (!$hide)
		{
			$this->status('Success')->interest(round(($data/100),2))->ok;
		}
		// return data
	...
	public function putSaving($action, $cid = null)
	{
		...
		if ($action == 'add')
		{
			is_set('ProductID:post&Amount:post', function($post) use ($info){
				...
				{
					$plan = DB::Products(get()->where('ID = ?')->bind($post->ProductID));
					...
					{
						$this->status('Error')->message("Invalid product Code.")->ok;
					}	
				}
				else
				{
					$this->status('Error')->message('Invalid ProductID')->ok;
				}
	...
	public function postWithdraw($type, $cid = null)
	{
				...
				is_set('Amount:post&PIN:post', function($data) use ($info){
					...
					if ($isAgent === false)
					{
						if (is_object($http->responseJson))
						{
							...
							$msg = $http->responseJson->msg ?? null;
							$status = $http->responseJson->status ?? null;
							$voucher = $http->responseJson->voucher ?? null;
							...
							elseif ($voucher !== null)
							{
								$this->status('Success')->message('Voucher generated.')->voucher($voucher)->ok;
							}
							else
							{
								$this->status('Error')->message("an error occured. please try again")->ok;
							}
						}
						else
						{
					...
					else
					{
						if (isset($http->responseJson->status))
						{
							if ($http->responseJson->status)
							{
								$this->status('Success')->voucher($http->responseJson->vouchers)->ok;
							}
							else
							{
								$this->status('Error')->message($http->responseJson->msg)->ok;
							}
						}
						else
						{
							$this->status('Error')->message("Sorry. Please try again.")->ok;
						}
					}
		...
		elseif ($type == 'bank')
		{
			...
			{
				$this->status('Error')->message('Invalid POST Field.')->ok;
			}
	...
	{	
		...
		if ($type == 'liquidate')
		{
			...
			{
				$this->status('Error')->message('Invalid POST Data.')->ok;
			} 
		}
		elseif ($type == 'verify')
		{
			if(isset($info->data->ID))
			{
				is_set('reference:post&amount:post', function($data){
					...
					{
						$this->status('Error')->message('Transaction Failed. Please try again later.')->ok;
					}
				});
		...
		{
			$this->status('Error')->request($type.' is invalid!')->ok;
		}
	...
	public function getDeposit($type, $cid = null)
	{
		...
		if ($type == 'recurrent')
		{
			if (isset($info->data->ID))
			{
				$all = DB::RecurrentDeposits(get()->where('CustomerID = ?')->bind($info->data->ID));
				...
				if ($all->rows > 0)
				{
					...
					$this->status('Success')->plans($savings)->ok;
				}
				else
				{
					$this->status('Error')->message('You have no active recurrent deposit savings.')->ok;
				}
			}
			else
			{
		...
		elseif ($type == 'card')
		{
			if (isset($info->data->ID))
			{
				if ($info->data->IsBankAccountConfirmed === 1)
				{
					$http = new Http('esusu.deposit.card');
					$http->get()->send();

					$this->status('Success')->trxRef($http->responseJson)->ok;
				}
				else
				{
					$this->status('Error')->message('You have not added a bank account. Cannot proceed with request.')->ok;
				}
			}
			else
			{
		...
		elseif ($type == 'cardMonthly')
		{
			if (isset($info->data->ID))
			{
				if ($info->data->IsBankAccountConfirmed === 1)
				{
					...
					if (is_object($http->responseJson))
					{
						$this->status('Success')->trxRef($http->responseJson->trxRef)->ok;
					}
					else
					{
						$this->status('Error')->message('Authentication failed.')->ok;
					}
				}
				else
				{
	...
	public function getReport($type, $cid = null)
	{
		...
		if ($cid !== null)
		{
			...
			if ($type == 'wallet')
			{
				if ($info->data->IsBankAccountConfirmed === 1)
				{
					if (isset($info->data->ID))
					{
						...
						if ($account->rows > 0)
						{
							...
							{
								$this->status('Error')->message('No Bank Transcation Report at this time')->ok;
							}
				...
				{
					$this->status('Error')->message('Bank Account not confirmed and cannot proceed.')->ok;
				}
			}
			elseif ($type == 'voucher')
			{
				if (isset($info->data->ID))
				{
					...
					$batches = DB::sql("SELECT * FROM VoucherBatches WHERE CustomerID = ? ORDER BY ID DESC OFFSET 0 ROWS FETCH NEXT $limit ROWS ONLY")->bind($info->data->ID)->run();
					...
					if ($batches->rows > 0)
					{
						...
						if ($batches->rows > 1)
						{
							...
							{
								$voucher = DB::sql("SELECT * FROM Vouchers WHERE BatchID = ?")->bind($b->ID)->run();
								...
								if ($voucher->rows > 0)
								{
									...
									$ch = curl_init('http://esusuonline.com.ng/ewalletAdmin/decryptvoucher/decrypt?decryptme='.$voucher->VoucherCode);
									...
									$data[$index]['Initiator'] = $info->data->Lastname.' '.$info->data->Othernames;
						...
						{
							$voucher = DB::sql("SELECT * FROM Vouchers WHERE BatchID = ?")->bind($batches->ID)->run();
							...
							if ($voucher->rows > 0)
							{
								...
								$data[0]['Initiator'] = $info->data->Lastname.' '.$info->data->Othernames;
						...
						$this->status('Success')->report($data)->ok;
					}
					else
					{
						$this->status('Error')->message('No Voucher Transcation Report at this time')->ok;
					}
	...
	public function postAgentVouchers($id)
	{
		...
		if ($info->ok)
		{
			is_set('Vouchers:post', function($post) use ($id, &$info){
				
				$voucher = $post->Vouchers;
				$Vouchers = [];
				$Vouchers[] = ['SN', 'Serial Number', 'Voucher Code'];
				$id = 1;
				foreach ($voucher as $index => $obj)
				{
					$Vouchers[] = [$id, $obj->SerialNumber, $obj->VoucherCode];
					$id++;
				}
				...
				// get email
				$email = $info->data->EmailAddress;

				// send email out.
				Plugins::mail()->send($email, 'Vouchers ready for download', 'Good day '.$info->Othernames.' '.$info->Lastname.'! Please click on this link to download your vouchers. '.$link);

				// send output
				$this->status('Success')->message('Request processed and vouchers has been sent to '.$email.' in an excel format. Please check your mailbox to download.')->ok;
			});
		}
		else
		{
	...
	public function getVoucher($method)
	{
		if ($method == 'download')
		{
			is_set('name:get', function($get){
				...
				{
					$this->status('Error')->message('Voucher Requested doesn\'t exists anymore.')->ok;
				}
			});
		}
		else
		{
			$this->status('Error')->message('Invalid Request.')->ok;
		}
	}
	...
	public function postSendticket($id)
	{
		...
		if ($info->ok)
		{
			...
			padding: 10px; display: table; width: 100%;">';
				...
				$this->status('Success')->message('Your ticket has been raised successfully. Please check you mail for a reply.')->ok;
	...
	{	
		...
		$topup += $rows;
		// return
		$this->status('Success')->users($topup)->ok;
	}
}
